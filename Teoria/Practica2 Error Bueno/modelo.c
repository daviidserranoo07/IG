...
int mod=GL_FILL;
bool iluminacio=true,dibujo=true;
...
void Malla::normalizar(){
  ...
  for(int i=0;i<caras_ply.size();i++){
    actual=caras_ply[i];
    modulo=sqrt(normalesVertice[actual*3]*normalesVertice[actual*3]+
                normalesVertice[actual*3+1]*normalesVertice[actual*3+1]+
                normalesVertice[actual*3+2]*normalesVertice[actual*3+2]);
    if(modulo>0.0){
...
void Superficie::cerrarFigura(){
  
  for(int j=0;j<size-1;j++){
    k=size-1*size+j;//Es el indice que indica que el vértice j de la instancia i donde se encontrara en el vector caras_ply
...
Superficie::Superficie(string path,int n):Malla(path){
...
void Superficie::draw(bool dibujo){
  if(dibujo) drawSmooth();
  else drawFlat();
}
...
Superficie superficie2("./plys/perfilCilindro.ply",100);
//Superficie superficie3("./plys/perfilCono.ply",100);
...
void Dibuja (void)
{
  ...
  ejesCoordenadas.draw(true);			// Dibuja los ejes
  ...
  glColor4fv(color);
  glTranslatef(-10,0,0);
  mallaPLY1.draw(dibujo);
  glColor4fv(color2);
  glTranslatef(25,0,0);
  mallaPLY2.draw(dibujo);
  glColor4fv(color3);
  glTranslatef(0,0,10);
  mallaPLY3.draw(dibujo);
  glColor4fv(color);
  glTranslatef(-15,0,-10);
  glColor4fv(color3);
  glTranslatef(0,0,0); 
  superficie.draw(dibujo);
  glTranslatef(-10,0,10); 
  superficie2.draw(dibujo);
  glTranslatef(10,0,0); 
  //superficie3.draw(dibujo);
